...
def gaussian(x, mean, std):
    return 1.0/np.sqrt(2.0*np.pi)/std*np.exp(-1.0*(x-mean)**2.0/(2.0*std**2.0))
...
def get_boost(table_l, table_r=None, rp=None, method='L-BFGS-B', returnfullparams=False):
    params_init = np.array([0.5,0.01]+[0.5 for i in range(len(rp))])
    ...
    res = minimize(fun, params_init, method=method, bounds=bnds,
